...
class AllBookingCubit extends Cubit<AllBookingState> {
  ...
  AllBookingCubit({required this.getAllBookingsUseCase})
      : super(AllBookingStatusInitial());

  static AllBookingCubit get(context) =>
  ...
  Future<void> getAllBookings(String type) async {
    ...
    emit(response.fold(
      ...
      (bookings) => AllBookingStatusComplete(
        upcomingBookings:
            bookings.where((element) => element.type == 'upcomming').toList(),
        cancelledBookings:
            bookings.where((element) => element.type == 'cancelled').toList(),
        completedBookings:
